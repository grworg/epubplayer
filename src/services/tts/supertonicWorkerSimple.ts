...
function logProbe(probe: MemoryProbe) {
  const heapInfo = probe.jsHeapUsedMB 
    ? `heap=${probe.jsHeapUsedMB}/${probe.jsHeapTotalMB}MB` 
    : 'heap=N/A'
  
  console.log(
    `[PROBE #${probe.inferenceNum}] ${heapInfo} | ` +
    `time=${probe.inferenceTimeMs}ms | samples=${probe.audioSamplesGenerated}`
  )
}
...
function analyzeProbes() {
  ...
  if (first.jsHeapUsedMB && last.jsHeapUsedMB) {
    ...
    console.log(`[ANALYSIS] Heap growth: ${heapGrowth.toFixed(2)}MB over ${memoryProbes.length} inferences`)
    console.log(`[ANALYSIS] Avg time first 5: ${avgTimeFirst5.toFixed(0)}ms, last 5: ${avgTimeLast5.toFixed(0)}ms`)
    
    if (heapGrowth > 50) {
      console.warn(`[ANALYSIS] ⚠️ SIGNIFICANT HEAP GROWTH DETECTED! Possible memory leak.`)
    }
    if (avgTimeLast5 > avgTimeFirst5 * 1.5) {
      console.warn(`[ANALYSIS] ⚠️ INFERENCE SLOWDOWN DETECTED! Performance degrading.`)
    }
  }
}
...
async function loadModels(
  ...
  if (device === 'webgpu') {
    ...
    useWebGPU = hasWebGPU
    if (!hasWebGPU) {
      console.warn('[Worker] WebGPU requested but not available, falling back to WASM')
    }
  }
  
  executionBackend = useWebGPU ? 'webgpu' : 'wasm'
  console.log(`[Worker] Using backend: ${executionBackend}`)
...
async function loadVoiceStyle(voiceId: string): Promise<{ ttl: any; dp: any }> {
  console.log(`[Worker] Loading voice style: ${voiceId}`)
  ...
  switch (message.type) {
    ...
    case 'cancel':
      if (message.requestId) {
        ...
        cancelAll = true
        console.log('[Worker] Cancel all requested')
        setTimeout(() => {
          cancelAll = false
          cancelledRequests.clear()
          console.log('[Worker] Cancel state auto-reset')
        }, 100)
      }
      break
  }
...
async function handleInit(message: InitMessage) {
  ...
  try {
    console.log('[Worker] Initializing (SIMPLE VERSION)...')
    ...
    console.log(`[Worker] Ready! Backend: ${executionBackend}, Voice: ${currentVoiceId}`)
    postMessage({ type: 'ready', backend: executionBackend })
  } catch (error) {
...
async function handleGenerate(message: GenerateMessage) {
  ...
  if (cancelAll || cancelledRequests.has(requestId)) {
    console.log('[Worker] Generation skipped - cancelled', { requestId, cancelAll })
  ...
  try {
    ...
    if (voiceId !== currentVoiceId) {
      console.log(`[Worker] Switching voice: ${currentVoiceId} -> ${voiceId}`)
    ...
    console.log(`[Worker] [#${inferenceNum}] Starting: "${text.substring(0, 50)}..." (${executionBackend})`)
    ...
    console.log(
      `[Worker] [#${inferenceNum}] Quality: silence=${(quality.silenceRatio * 100).toFixed(1)}% ` +
      `maxAmp=${quality.maxAmplitude.toFixed(3)} avgAmp=${quality.avgAmplitude.toFixed(3)} ` +
      `${quality.suspiciousPattern ? '⚠️ SUSPICIOUS' : '✓ OK'}`
    )
    
    // Warn if quality looks bad
    if (quality.suspiciousPattern) {
      console.warn(`[Worker] [#${inferenceNum}] ⚠️ Audio quality appears degraded!`)
    }
    ...
    console.log(`[Worker] [#${inferenceNum}] Generated ${actualDuration.toFixed(2)}s in ${inferenceTime}ms`)
...
async function handleSetVoice(message: SetVoiceMessage) {
  ...
  try {
    console.log(`[Worker] Setting voice: ${message.voiceId}`)
    currentStyle = await loadVoiceStyle(message.voiceId)
    currentVoiceId = message.voiceId
  } catch (error) {
    console.error('[Worker] Failed to set voice:', error)
    postMessage({
      type: 'error',
